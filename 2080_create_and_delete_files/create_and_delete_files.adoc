...
Instead of moving files one by one, it is possible to move multiple files at once. Here, the `\*` character is used to match any file that has the .txt extension. The `*` character is a commond wildcard characters. Wildcard characters are used for pattern matching.
